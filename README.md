...
- 双击`1点这里.bat`(或`Fhoe-Rail.bat`)打开菜单，按提示运行
- 或运行`Fhoe-Rail.exe`直接免配置开启
- WebUI 控制面板：双击`Start-WebUI.bat`，浏览器自动打开 http://127.0.0.1:8666
...
内置本地 WebUI 控制面板，零依赖（仅 Python 标准库），方便查看运行状态、管理地图与配置。

- 启动：双击`Start-WebUI.bat`（自动管理员权限），浏览器自动打开 http://127.0.0.1:8666
- 指挥台：运行状态与统计（战斗次数/总用时/疾跑节约/卡顿）
- 运行控制：从浏览器启动/停止锄地（默认/白名单/选图/开发者选图/录制/安装依赖 六种模式），实时输出
- 地图图鉴：按版本/星球浏览全部路线，支持名称/编号/作者搜索
- 参数设置：config.json 全部配置项可视化编辑，支持保存/复制/下载
- 航行日志：实时查看运行日志
- 新图注意：浏览注意事项图片

## 通知推送

支持 19 种通知渠道，在 WebUI「参数设置 → 通知推送」中配置：

- Windows 桌面通知（零配置，默认）/ Server酱 / Server酱³ / PushPlus / Telegram / Bark / 钉钉 / 企业微信机器人/应用 / 飞书 / Discord / Gotify / Ntfy / PushDeer / Qmsg / SMTP邮件 / QQ机器人 / 自定义请求 / 通用Webhook
- 通知时机：开始锄地 / 锄地完成（含统计摘要）/ 运行出错，可分别开关
- WebUI 中可直接发送测试通知验证配置
...
| allowlist_mode_once      | 是否仅此次运行白名单地图                                               |
| auto_shutdown            | 锄地结束后系统操作：0无操作 1关机 2注销 3关闭指定进程                  |
| taskkill_name            | auto_shutdown为3时关闭的进程名                                         |
| webhook_url              | 通用Webhook通知地址                                                     |
| notify_enabled           | 是否启用通知推送                                                       |
| notify_channel           | 通知渠道：windows桌面通知/serverchan/pushplus/telegram/dingtalk等       |
| notify_key               | 通知渠道密钥                                                           |
| notify_chat_id           | 通知目标参数(如telegram的chat_id)                                      |
| notify_params            | 通知渠道高级参数(JSON)                                                 |
| notify_on_start          | 开始锄地时通知                                                         |
| notify_on_end            | 锄地完成时通知(含统计)                                                 |
| notify_on_error          | 运行出错时通知                                                         |
